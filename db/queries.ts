...
export async function upsertSiteSettings(data: Partial<Omit<SiteSettings, "id">>): Promise<void> {
  ...
  if (existing) {
    ...
    if (updates.length > 0) {
      ...
      await db.execute(`UPDATE site_settings SET ${updates.join(", ")} WHERE id = ?${paramIndex}`, values);
    }
  } else {
...
export async function createPost(data: Omit<Post, "id" | "created_at" | "updated_at">): Promise<void> {
  const keys = Object.keys(data);
  const placeholders = keys.map((_, i) => `?${i + 1}`).join(", ");
  const values = Object.values(data);
...
export async function updatePost(id: number, data: Partial<Omit<Post, "id" | "created_at" | "updated_at">>): Promise<void> {
  const updates: string[] = [];
  const values: any[] = [];
  let paramIndex = 1;
...
export async function createPage(data: Omit<Page, "id" | "created_at" | "updated_at">): Promise<void> {
  const keys = Object.keys(data);
  const placeholders = keys.map((_, i) => `?${i + 1}`).join(", ");
  const values = Object.values(data);
...
export async function updatePage(id: number, data: Partial<Omit<Page, "id" | "created_at" | "updated_at">>): Promise<void> {
  const updates: string[] = [];
  const values: any[] = [];
  let paramIndex = 1;
...
export async function createContactSubmission(data: Omit<ContactSubmission, "id" | "created_at">): Promise<void> {
  const keys = Object.keys(data);
  const placeholders = keys.map((_, i) => `?${i + 1}`).join(", ");
  const values = Object.values(data);
...
export async function updateContactSubmission(id: number, data: Partial<Omit<ContactSubmission, "id" | "created_at">>): Promise<void> {
  const updates: string[] = [];
  const values: any[] = [];
  let paramIndex = 1;
  ...
  await db.execute(`UPDATE contact_submissions SET ${updates.join(", ")} WHERE id = ?${paramIndex}`, values);
}
...
export async function createRedirect(data: Omit<Redirect, "id" | "created_at">): Promise<void> {
  ...
  await db.execute(
    `INSERT INTO redirects (${keys.join(", ")}) VALUES (${placeholders})`,
    values
  );
}

export async function updateRedirect(id: number, data: Partial<Omit<Redirect, "id" | "created_at">>): Promise<void> {
  const updates: string[] = [];
  const values: any[] = [];
  let paramIndex = 1;
